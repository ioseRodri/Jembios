...
export default function Admin() {
  ...
  const deleteProduct = async (id) => {
    const ok = window.confirm(`¿Eliminar el producto #${id}? Esta acción es permanente.`);
    if (!ok) return;
    try {
      ...
      const msg = e?.response?.data?.error || "No se pudo eliminar (puede estar referenciado en órdenes)";
      setToast(msg);
      console.error(e);
    }
  };
  ...
  const fetchTasksData = async () => {
    try {
      const [emps, orders, tasks] = await Promise.all([
        ...
        http.get("/api/orders", { params: { status: "activas" } }).then((r) => r.data),
  ...
  const createTask = async (e) => {
    ...
    const hasActiveEmp = taskEmployees.some(e => e.status !== "INACTIVO" && Number(e.id) === employeeIdNum);
    if (!hasActiveEmp) {
  ...
  return (
    <div className="admin-wrap">
      ...
      <aside className="admin-side">
        ...
        <button className="side-btn" onClick={() => setView("pendientes")}>Productos para alta</button>
        <button className="side-btn" onClick={() => setView("form")}>Añadir Producto</button>
        <hr style={{ margin: '12px 0', opacity: 0.3 }} />
        
        <button className="side-btn" onClick={() => setView("empleados")}>Registrar empleados</button>
        
        <button className="side-btn" onClick={() => setView("tareas")}>Asignar tareas</button>
        <button className="side-btn" onClick={() => setView("historial")}>Historial de tareas</button>
        <button className="side-btn" onClick={() => setView("productos")}>Productos registrados</button>
        <button className="side-btn" onClick={() => setView("ordenes")}>Historial de órdenes</button>
      </aside>
              ...
              <div className="admin-grid">
                {pendientes.map((p) => (
                  <div key={p.id_pendiente} className="admin-card">
                    ...
                    {p.imagen_url && (
                      <img src={`${uploadsBase}${p.imagen_url}`} alt={p.nombre} className="admin-img" />
                    )}
                    {p.certificado_url && (
                      <a href={`${uploadsBase}${p.certificado_url}`} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', textDecoration: 'underline', display: 'inline-block', marginTop: 8 }}>Ver certificado</a>
                    )}
                    <div className="admin-actions">
                      <button className="btn-approve" onClick={() => aprobarProducto(p.id_pendiente)}>Aprobar</button>
                      <button className="btn-reject" onClick={() => rechazarProducto(p.id_pendiente)}>Rechazar</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <form className="admin-form" onSubmit={handleSubmitProducto}>
              ...
              {imagen && (
                <div className="preview-container">
                  <img src={URL.createObjectURL(imagen)} alt="Previsualización" className="preview-img" />
                </div>
              )}
        ...
        {view === 'empleados' && (
          <>
            ...
            <form className="admin-form" onSubmit={createEmployee}>
              <div className="form-row">
                <input type="text" placeholder="Nombre del empleado" value={empName} onChange={(e) => setEmpName(e.target.value)} required />
                ...
                <label className="checkbox-label">
                  <input type="checkbox" checked={empStatus} onChange={(e) => setEmpStatus(e.target.checked)} />
                  Activo
                </label>
              </div>
                  ...
                  <tbody>
                    {empList.map((e) => (
                      <tr key={e.id}>
                        ...
                        <td>{e.role}</td>
                        <td>
                          <span className={`badge ${e.status === 'ACTIVO' ? 'badge-green' : 'badge-gray'}`}>{e.status}</span>
                        </td>
                        <td>
                          <button className="btn-link" onClick={() => toggleEmployeeStatus(e)}>
        ...
        {view === 'tareas' && (
          <>
            ...
            <form className="admin-form" onSubmit={createTask}>
              <div className="form-row">
                <select value={selEmployeeId} onChange={(e) => setSelEmployeeId(e.target.value)} required>
                  {taskEmployees.filter((e) => e.status !== 'INACTIVO').map((e) => {
                ...
                <select value={selOrderId} onChange={(e) => setSelOrderId(e.target.value)} required>
                  {taskOrders.map((o) => (
                    <option key={o.id} value={o.id}>#{o.id} — {o.name || 'Sin nombre'} — {o.status}</option>
                  ))}
                </select>
                <select value={selType} onChange={(e) => setSelType(e.target.value)} required>
                ...
                <table className="admin-table">
                  ...
                  <tbody>
                    {taskList.slice(0, 10).map((t) => {
                      const isSystem = !t.assignee_id || t.assignee_id === 0 || !t.assignee_name;
                      const displayName = isSystem ? 'Sistema' : t.assignee_name;
                      const displayType = String(t.type).toLowerCase() === 'picking' ? 'Agendado' : t.type;
                      const computedUiStatus = String(t.type).toLowerCase() === 'picking'
                        ? 'CUMPLIDO'
                      ...
                      return (
                        <tr key={t.id}>
                          <td>{t.id}</td>
                          <td>#{t.order_id}</td>
                          <td>{displayName}{!isSystem && <span className="muted"> ({t.assignee_id})</span>}</td>
                          <td>{displayType}</td>
                          <td><span className={`badge ${isDone ? 'badge-green' : 'badge-yellow'}`}>{computedUiStatus}</span></td>
                          <td>
                            {!isDone ? (
                              <button className="btn-approve" onClick={() => completeTask(t.id)}>Finalizado</button>
                            ) : (
                  ...
                  <tbody>
                    {prodList.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td>{p.price != null ? `S/ ${Number(p.price).toFixed(2)}` : '-'}</td>
                        <td>{p.stock != null ? p.stock : '-'}</td>
                        <td>
                          <span className={`badge ${p.active ? 'badge-green' : 'badge-gray'}`}>{p.active ? 'ACTIVO' : 'INACTIVO'}</span>
                        </td>
                        <td>
                          <button className="btn-reject" onClick={() => deleteProduct(p.id)} title="Eliminar producto">Eliminar</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <>
            ...
            {!ordersLoading && !ordersErr && (
              <>
                ...
                <button className="btn-link" style={{ marginBottom: 16 }} onClick={() => setShowGraphModal(true)}>Mostrar gráficas</button>
                {orders.length === 0 ? (
                  <p style={{ color: '#4b5563' }}>No hay órdenes registradas.</p>
                ) : (
                  <div className="admin-table-wrap">
                    <table className="admin-table">
                      <thead>
                      ...
                      <tbody>
                        {orders.map((o) => (
                          <tr key={o.id}>
                            ...
                            <td>
                              <button className="btn-link" onClick={() => openOrderDetail(o.id)}>Ver detalle</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div className="modal-overlay" onClick={() => setShowGraphModal(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                      <div className="drawer-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button className="btn-link" onClick={() => setShowGraphModal(false)}>Cerrar</button>
                      </div>
                      <div style={{ marginTop: 12 }}>
                        
                        <h3 className="admin-subtitle" style={{ marginBottom: 4 }}>Cantidad de ventas por día</h3>
                        ...
                        <h3 className="admin-subtitle" style={{ marginTop: 24, marginBottom: 4 }}>Ganancia total por día</h3>
                        <div style={{ position: 'relative', width: '100%', height: 240 }}>
                          <Bar
                  ...
                  <div className="drawer-overlay" onClick={closeOrderDetail}>
                    <aside className="drawer-panel" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
                      <div className="drawer-header">
                        <h2 className="admin-subtitle">Orden #{detailData?.order?.id}</h2>
                        <button className="btn-link" onClick={closeOrderDetail}>Cerrar</button>
                      </div>
                      <div className="drawer-section">
                        <div><b>Cliente:</b> {detailData?.order?.customer_name || '—'}</div>
                        <div><b>Estado:</b> {detailData?.order?.status}</div>
                        <div><b>Subtotal:</b> S/ {Number(detailData?.order?.subtotal || 0).toFixed(2)}</div>
                        <div><b>Envío:</b> S/ {Number(detailData?.order?.shipping_cost || 0).toFixed(2)}</div>
                        <div><b>Descuento:</b> S/ {Number(detailData?.order?.discount_total || 0).toFixed(2)}</div>
                        <div><b>Total:</b> S/ {Number(detailData?.order?.total_amount || 0).toFixed(2)}</div>
                        {detailData?.order?.created_at && (
                          <div><b>Fecha:</b> {fmtLima(detailData.order.created_at)}</div>
                        )}
                      </div>
                      <h3 className="admin-subtitle" style={{ marginTop: 12 }}>Items</h3>
                            ...
                            <tbody>
                              {detailData.items.map((it) => (
                                <tr key={it.id}>
                                  ...
                                  <td>S/ {Number(it.unit_price * it.qty).toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
